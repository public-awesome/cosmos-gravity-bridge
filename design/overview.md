...
* `Observed` - events on Ethereum are considered `Observed` when 66% of the active Cosmos validator set during a given block has submitted an oracle message attesting to seeing the event. Note this is distinct from `Confirmed`! At any given time the set of who can `Observe` an event and who can `Confirm` and event will be slightly different. Since `Confirmed` is based off of the `Last ValSet` and `Observed` is always up to date with the latest validator set. `Confirmed` has to do with bringing information from Cosmos to Ethereum, while `Observed` has to do with bringing information from Ethereum to Cosmos.
* `EthBlockData` - This is a message sent by `Orchestrators` to the `Peggy module` that contains Ethereum state we are interested in from a given Ethereum block. Right now, this data is all the public variables of the Peggy contract.
...
## Observing Ethereum state ("Ethereum Oracle")

The `Peggy module` needs to know various things about what is happening with the Ethereum contract, for instance when someone has locked some tokens to send over to Cosmos, or when a batch of transactions has been relayed the other direction. It's simpler if we deal with the logic of `Observing` Ethereum state in one spot in the module instead of dealing with it multiple times in all the places it is referenced. The state `Observed` here can be saved in the Cosmos KV store to be made generally available.

* There is a governance parameter called EthBlockDelay, for example 50 blocks
* The `Orchestrator` binary on each `Validator` gets the current block number from their Ethereum full node, then gets the data we are interested in from the block `EthBlockDelay` lower than the current one. Right now, this data is just all public variables of the Peggy contract, but t`here may be other things as well.
...
* Once there is a version of the `EthBlockData` that matches from at least 66% of the validators (the current validators in power on the Cosmos chain, not the last valset in the contract), it is considered `Observed`. For example, another part of the code might mint some tokens in Cosmos based on tokens sent into the Peggy contract.
...
* Once enough signatures are collected and the batch is `Confirmed` any `Orchestrator` can collect these signatures from the REST endpoint, assemble an Ethereum transaction and submit the batch. Many `Orchestrator`s that are not part of the `Validator` set may compete to submit batches and profit from the transaction fees.
* Once a batch has been `Observed` on Ethereum, the batch is removed from the `Peggy Bridge Tx pool` and the MsgSendToEth flow is now fully complete.
