...
contract Peggy {
    ...
    uint256 public state_lastWithdrawalBatchNonce = 0;

    // These are set once at initialization
    address public state_tokenContract;
    bytes32 public state_peggyId;
    uint256 public state_powerThreshold;
    uint256 public state_tokenDepositNonce;

    event ValsetUpdatedEvent(uint256 _nonce, address[] _validators, uint256[] _powers);
    event TransferOutEvent(uint256 _nonce, address _erc20ContractAddress, string _symbol, string _destination, uint256 _amount);
    event BootstrapEvent(uint256 _nonce, bytes32 _peggyId, uint256 _powerThreshold, address[] _validators, uint256[] _powers);
    event WithdrawalBatchEvent(uint256 _nonce);
    ...
    // These are arrays of the parts of the current validator's signatures
        ...
        console.log("auth valset checkpoint");
    ...
    // The batch of transactions
        ...
        require(
            _batch_nonce > state_lastWithdrawalBatchNonce,
            "Withdrawal batch nonce must be higher than last nonce and strictly increasing"
        );


        state_lastWithdrawalBatchNonce = _batch_nonce;
        ...
        console.logBytes32(transactionsHash);
        ...
        emit WithdrawalBatchEvent(_batch_nonce);
    }
    ...
    function transferOut(string memory _destination, uint256 _amount) public {
        ...
        emit TransferOutEvent(++state_tokenDepositNonce, state_tokenContract, ERC20(state_tokenContract).symbol(), _destination, _amount);
    }
    ...
    // The validator set
        ...
        uint32 initialNonce = 1;
        bytes32 newCheckpoint = makeCheckpoint(_validators, _powers, initialNonce, _peggyId);
        ...
        state_lastCheckpoint = newCheckpoint;
        state_lastValsetNonce = initialNonce;

        emit BootstrapEvent(initialNonce, _peggyId, _powerThreshold, _validators, _powers);
    }
}
