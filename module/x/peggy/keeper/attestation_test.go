...
func TestObserveDeposit(t *testing.T) {
	...
	gotAttestation = k.GetLastProcessedAttestation(ctx, types.ClaimTypeEthereumBridgeDeposit)
	assert.Equal(t, exp, *gotAttestation)
}
...
func TestObserveWithdrawBatch(t *testing.T) {
	var (
		...
		myValAddr                         = sdk.ValAddress(myOrchestratorAddr) // revisit when proper mapping is impl in keeper
		myBlockTime                       = time.Date(2020, 9, 14, 15, 20, 10, 0, time.UTC)
		batchNonce                        = types.NewUInt64Nonce(1)
	)
	k, ctx, _ := CreateTestEnv(t)
	k.StakingKeeper = NewStakingKeeperMock(myValAddr)
	k.storeBatch(ctx, batchNonce, types.OutgoingTxBatch{
		BatchStatus: types.BatchStatusSubmitted,
	})

	// when
	ctx = ctx.WithBlockTime(myBlockTime)
	gotAttestation, err := k.AddClaim(ctx, types.ClaimTypeEthereumBridgeWithdrawalBatch, batchNonce, myValAddr, nil)
	// then
	require.NoError(t, err)

	// and claim persisted
	claimFound := k.HasClaim(ctx, types.ClaimTypeEthereumBridgeWithdrawalBatch, batchNonce, myValAddr, nil)
	assert.True(t, claimFound)
	...
	assert.Equal(t, exp, *gotAttestation)
	// and last observed status updated
	gotAttestation = k.GetLastProcessedAttestation(ctx, types.ClaimTypeEthereumBridgeWithdrawalBatch)
	assert.Equal(t, exp, *gotAttestation)
}
...
func TestObserveBridgeMultiSigUpdate(t *testing.T) {
	...
	assert.Equal(t, exp, *gotAttestation)
	// and last observed status updated
	gotAttestation = k.GetLastProcessedAttestation(ctx, types.ClaimTypeEthereumBridgeMultiSigUpdate)
	assert.Equal(t, exp, *gotAttestation)
	gotNonce := k.GetLastAttestedNonce(ctx, types.ClaimTypeEthereumBridgeMultiSigUpdate)
...
func TestObserveBridgeBootstrap(t *testing.T) {
	...
	gotAttestation, err := k.AddClaim(ctx, types.ClaimTypeEthereumBridgeBootstrap, myNonce, myValAddr, details)
	// then
	require.NoError(t, err)

	// and claim persisted
	claimFound := k.HasClaim(ctx, types.ClaimTypeEthereumBridgeBootstrap, myNonce, myValAddr, details)
	assert.True(t, claimFound)

	// and expected state
	exp := types.Attestation{
		ClaimType:     types.ClaimTypeEthereumBridgeBootstrap,
		Nonce:         myNonce,
		Certainty:     types.CertaintyObserved,
		Status:        types.ProcessStatusProcessed,
		ProcessResult: types.ProcessResultSuccess,
	...
	assert.Equal(t, exp, *gotAttestation)
	// and last observed status updated
	gotAttestation = k.GetLastProcessedAttestation(ctx, types.ClaimTypeEthereumBridgeBootstrap)
	assert.Equal(t, exp, *gotAttestation)
	gotNonce := k.GetLastAttestedNonce(ctx, types.ClaimTypeEthereumBridgeBootstrap)
	require.NotNil(t, gotNonce)
}
...
func TestApproveBridgeMultiSigUpdate(t *testing.T) {
	...
	assert.Equal(t, exp, *gotAttestation)
	// and last observed status updated
	gotAttestation = k.GetLastProcessedAttestation(ctx, types.ClaimTypeOrchestratorSignedMultiSigUpdate)
	assert.Equal(t, exp, *gotAttestation)

	gotNonce := k.GetLastAttestedNonce(ctx, types.ClaimTypeOrchestratorSignedMultiSigUpdate)
	require.NotNil(t, gotNonce)
	assert.Equal(t, myNonce, *gotNonce)
}
