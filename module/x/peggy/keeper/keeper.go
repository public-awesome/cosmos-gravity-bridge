...
func (k Keeper) SetValsetRequest(ctx sdk.Context) *types.Valset {
	...
	event := sdk.NewEvent(
		...
		sdk.NewAttribute(types.AttributeKeyMultisigID, string(valset.Nonce)),
		sdk.NewAttribute(types.AttributeKeyNonce, string(valset.Nonce)),
	)
...
func (k Keeper) SetBootstrapValset(ctx sdk.Context, valset *types.Valset) error {
	...
	event := sdk.NewEvent(
		...
		sdk.NewAttribute(types.AttributeKeyContract, k.GetBridgeContractAddress(ctx).String()),
		sdk.NewAttribute(types.AttributeKeyBridgeChainID, strconv.Itoa(int(k.GetBridgeChainID(ctx)))),
		sdk.NewAttribute(types.AttributeKeyMultisigID, string(nonce)),
...
func (k Keeper) SetBridgeApprovalSignature(ctx sdk.Context, signType types.SignType, nonce uint64, validator sdk.ValAddress, signature []byte) []byte {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetBridgeApprovalSignature(ctx sdk.Context, signType types.SignType, nonce uint64, validator sdk.ValAddress) []byte {
	store := ctx.KVStore(k.storeKey)
	return store.Get(types.GetBridgeApprovalSignatureKey(signType, nonce, validator))
}

func (k Keeper) HasBridgeApprovalSignature(ctx sdk.Context, signType types.SignType, nonce uint64, validator sdk.ValAddress) bool {
	store := ctx.KVStore(k.storeKey)
	return store.Has(types.GetBridgeApprovalSignatureKey(signType, nonce, validator))
}

func (k Keeper) IterateBridgeApprovalSignatures(ctx sdk.Context, signType types.SignType, nonce uint64, cb func(_ []byte, sig []byte) bool) {
...
func (k Keeper) SetValsetConfirm(ctx sdk.Context, valsetConf types.MsgValsetConfirm) {
	store := ctx.KVStore(k.storeKey)
	// TODO: does this need to be a ValAddressFromBech32?
	addr, err := sdk.AccAddressFromBech32(valsetConf.Validator)
	if err != nil {
		panic(err)
	}
	store.Set(types.GetValsetConfirmKey(valsetConf.Nonce, addr), k.cdc.MustMarshalBinaryBare(valsetConf))
}
...
func (k Keeper) SetEthAddress(ctx sdk.Context, validator sdk.ValAddress, ethAddr []byte) {
	store := ctx.KVStore(k.storeKey)
	store.Set(types.GetEthAddressKey(validator), ethAddr)
}

func (k Keeper) GetEthAddress(ctx sdk.Context, validator sdk.ValAddress) []byte {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetCurrentValset(ctx sdk.Context) *types.Valset {
	...
	nonce := uint64(uint64(ctx.BlockHeight()))
